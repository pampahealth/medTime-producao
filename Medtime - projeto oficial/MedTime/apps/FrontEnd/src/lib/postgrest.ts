...
export function createPostgrestClient(userToken?: string, schemaOverride?: string) {
  const schema = schemaOverride ?? POSTGREST_SCHEMA;
  ...
  const bearerToken = userToken || (POSTGREST_API_KEY || "");
  if (bearerToken) {
    client.headers.set("Authorization", `Bearer ${bearerToken}`);
  }
